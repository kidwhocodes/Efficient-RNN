    ...
    with torch.no_grad():
        for x_batch, y_batch in batches:
            ...
            if last_only:
                ...
                N = y_batch[-1].numel()
                correct = (pred == y_batch[-1]).sum().item()
            else:
...
def run_prune_experiment(
    ...
    **kwargs,
    ...
    if task == "synthetic":
        ...
        data = NeuroGymDM(env, T=T, B=B, device=device, last_only=False, seed=config.seed)
        task_meta.update({
            "env": task_name,
            "T": T,
            "B": B,
    ...
    def run_eval(steps: int, offset: int) -> Tuple[float, float]:
        if fixed_batches is not None:
            return evaluate_on_fixed_batches(model, fixed_batches, criterion, last_only=last_only)
        seed_val = None if eval_seed_base is None else int(eval_seed_base) * 10 + offset
        with temporary_seed(seed_val):
            return evaluate(model, data, device, criterion, steps=steps, last_only=last_only)

    # ------------------------------------------------------------------
    # Phase A: baseline evaluation
    # ------------------------------------------------------------------
    pre0_loss, pre0_acc = run_eval(eval_steps_pre0, 0)
    ...
    if not skip_training and train_steps > 0:
        ...
        pre_loss, pre_acc = run_eval(eval_steps_pre, 1)
    ...
    post_loss, post_acc = run_eval(eval_steps_post, 3)
